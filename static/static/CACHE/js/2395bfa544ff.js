...
$spy.affix(data)})})}(window.jQuery);!function($){"use strict";var Lightbox=function(element,options)
...
{if(img)
{$target.find('.lightbox-content').html($target.data('original-content'));img=undefined;}});})}(window.jQuery);!function($){"use strict";var Typeahead=function(element,options){this.$element=$(element)
this.options=$.extend({},$.fn.typeahead.defaults,options)
this.matcher=this.options.matcher||this.matcher
this.sorter=this.options.sorter||this.sorter
this.highlighter=this.options.highlighter||this.highlighter
...
this.shown=false
this.listen()}
Typeahead.prototype={constructor:Typeahead,select:function(){var val=this.$menu.find('.active').data('item')
this.$element.val(this.updater(val)).change()
return this.hide()},textproperty:function(item){return item},updater:function(item){return this.textproperty(item)},show:function(){var pos=$.extend({},this.$element.offset(),{height:this.$element[0].offsetHeight})
this.$menu.css({top:pos.top+pos.height,left:pos.left})
this.$menu.show()
this.shown=true
return this},hide:function(){this.$menu.hide()
this.shown=false
return this},_delaySource:function(query,source,process){if(this.calledOnce===undefined){this.calledOnce=true
var items=source(query,process)
if(items){this.delay=0
return items}}else{if(this.delay===0){return source(query,process)}else{if(this.timeout){clearTimeout(this.timeout)}
this.timeout=setTimeout(function(){return source(query,process);},this.delay)}}},lookup:function(event){var items
this.query=this.$element.val()
if(!this.query||this.query.length<this.options.minLength){return this.shown?this.hide():this}
...
items=this.sorter(items)
if(!items.length){return this.shown?this.hide():this}
return this.render(items.slice(0,this.options.items)).show()},matcher:function(item){return~this.textproperty(item).toLowerCase().indexOf(this.query.toLowerCase())},sorter:function(items){var beginswith=[],caseSensitive=[],caseInsensitive=[],item
while(item=items.shift()){var itemtext=this.textproperty(item);if(!itemtext.toLowerCase().indexOf(this.query.toLowerCase()))beginswith.push(item)
else if(~itemtext.indexOf(this.query))caseSensitive.push(item)
else caseInsensitive.push(item)}
return beginswith.concat(caseSensitive,caseInsensitive)},highlighter:function(item){var query=this.query.replace(/[\-\[\]{}()*+?.,\\\^$|#\s]/g,'\\$&')
return this.textproperty(item).replace(new RegExp('('+query+')','ig'),function($1,match){return'<strong>'+match+'</strong>'})},render:function(items){var that=this
items=$(items).map(function(i,item){i=$(that.options.item).data('item',item)
i.find('a').html(that.highlighter(item))
return i[0]})
items.first().addClass('active')
this.$menu.html(items)
